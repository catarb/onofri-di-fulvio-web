...
import { MapPin, Clock, Phone, ExternalLink, MessageCircle, Star } from "lucide-react";
        ...
        <div className="grid gap-20 lg:grid-cols-[0.9fr_1.1fr] items-start">
          ...
          >
            ...
            >
              {visitData.map((item) => (
                <motion.div
                  key={item.label}
                  variants={cardItem}
                  className="group glass premium-card rounded-[32px] p-7"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-6">
                      ...
                      className="premium-card-icon hidden sm:flex h-12 w-12 items-center justify-center rounded-full border border-ink/5 text-ink/25 hover:bg-aqua hover:text-white hover:border-aqua transition-all duration-300"
                    >
            ...
            <motion.div
              ...
              className="mt-12 p-8 glass consultorio-facade-card rounded-[40px] border-t-4 border-t-ink/20"
            >
               <div className="flex items-center justify-between flex-wrap gap-6">
                  ...
                  <a 
                    href="https://www.google.com/maps/search/?api=1&query=Onofri-Di+Fulvio+Odontología+Santa+Rosa"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-3 rounded-full bg-ink px-8 py-4 text-sm font-bold text-white shadow-premium hover:opacity-90 transition-all active:scale-95"
                  >
          ...
          >
            <div className="relative aspect-video lg:aspect-[4/5] w-full rounded-[48px] overflow-hidden shadow-premium border border-white group">
               ...
               {/* Botón flotante para abrir en maps (mobile) */}
               <div className="absolute bottom-6 right-6 sm:hidden">
                  <a 
                    href="https://www.google.com/maps/search/?api=1&query=Av.+Uruguay+785,+Santa+Rosa,+La+Pampa"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex h-14 w-14 items-center justify-center rounded-full bg-white text-teal shadow-premium"
                  >
                    <MapPin size={24} />
                  </a>
               </div>
